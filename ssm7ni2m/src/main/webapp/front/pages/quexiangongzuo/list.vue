<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.xiangmuzhuangtai" type="text" placeholder="项目状态" ></input>
			</view>
                              			<view v-if="queryIndex==2" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.gengxinriqi" type="text" placeholder="更新日期" ></input>
			</view>
        ...
        <view :style='categoryName===item.xiangmuleixing?sactiveItem:sitem' class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.xiangmuleixing?'tabActive':''" @tap="categoryClick(item.xiangmuleixing)">{{item.xiangmuleixing}}</view>
      </scroll-view>
        ...
        <view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"0px 20rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 1% 20rpx 2%","borderColor":"rgba(130, 163, 157, 1)","backgroundColor":"#fff","borderRadius":"20rpx","borderWidth":"2rpx","width":"47%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
                                <view class="uni-product-title" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"28rpx","lineHeight":"56rpx","borderStyle":"solid"}'>{{product.xiangmumingcheng}}</view>
                                                              <view class="image-view">
              <image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"8rpx","borderWidth":"0","width":"95%","borderStyle":"solid","height":"260rpx"}' mode="aspectFill" class="uni-product-image" :src="product.xiangmutupian?baseUrl+product.xiangmutupian.split(',')[0]:''"></image>
            </view>
                                                                                                                                                                          <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('quexiangongzuo','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('quexiangongzuo','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
      </view>
            <!-- 样式2 -->
...
<script>
	export default {
		...
		async onShow() {
			...
			let res = await this.$api.list('xiangmuleixing', {page:1,limit:100});
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.xiangmuzhuangtai="";
				this.searchForm.gengxinriqi="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.xiangmuzhuangtai){
					params['xiangmuzhuangtai'] = '%' + this.searchForm.xiangmuzhuangtai + '%'
				}
				if(this.searchForm.gengxinriqi){
					params['gengxinriqi'] = '%' + this.searchForm.gengxinriqi + '%'
				}
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('quexiangongzuo', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.xiangmuzhuangtai){
					searchForm['xiangmuzhuangtai'] = '%' + this.searchForm.xiangmuzhuangtai + '%'
				}
				if(this.searchForm.gengxinriqi){
					searchForm['gengxinriqi'] = '%' + this.searchForm.gengxinriqi + '%'
				}
				let res = await this.$api.list(`quexiangongzuo`, searchForm);
				// 如果是第一页数据置空
